...
{
	Vector3 pos = Vector3(m_positionX, m_positionY, m_positionZ);
	return pos;
}
...
void CameraClass::TransformCoord(Vector3& vector, float* matrix)
{
	float x, y, z;
...
void CameraClass::BuildViewMatrix(Vector3 position, Vector3 lookAt, Vector3 up)
{
	...
	mViewMatrix->elements[0]  = xAxis.x;
	mViewMatrix->elements[1]  = yAxis.x;
	mViewMatrix->elements[2]  = zAxis.x;
	mViewMatrix->elements[3]  = 0.0f;

	mViewMatrix->elements[4]  = xAxis.y;
	mViewMatrix->elements[5]  = yAxis.y;
	mViewMatrix->elements[6]  = zAxis.y;
	mViewMatrix->elements[7]  = 0.0f;

	mViewMatrix->elements[8]  = xAxis.z;
	mViewMatrix->elements[9]  = yAxis.z;
	mViewMatrix->elements[10] = zAxis.z;
	mViewMatrix->elements[11] = 0.0f;

	mViewMatrix->elements[12] = result1;
	mViewMatrix->elements[13] = result2;
	mViewMatrix->elements[14] = result3;
	mViewMatrix->elements[15] = 1.0f;

	return;
}

Matrix4x4 CameraClass::GetViewMatrix()
{
	Matrix4x4 viewMatrix = *mViewMatrix;
